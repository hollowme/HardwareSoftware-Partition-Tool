package gui;
//均链接
public class ResultofMMM3 extends ResultFather{
	
	ResultofMMM3(int tasknumber,int modulenumber){
			...
			for(int i=0;i<n&&absk>pn;i++) {
				if (setno[i]!=0) {
					for (int j = i + 1; j < n&&absk>pn; j++) {
						if (setno[j]!=0) {
							double meand = 0;//两个集合的相似度
							int communicateroute=0;
							for (int a = 0; a < n; a++) {
								if (K[a]==i) {
									for (int b = 0; b < n ; b++) {
										if (K[b]==j) {
											meand = matrix[a][b];//更新相似度
											communicateroute += 1;
										}
									} 
								}
							}
							meand /= communicateroute;
							if (meand >= thCost && setno[i]+setno[j] <= m) {//条件满足，合并集合i,j
